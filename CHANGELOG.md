...
- **Migrated from ESLint to [Oxlint](https://oxc.rs/docs/guide/usage/linter).** Presets are now distributed as `.oxlintrc.json` files instead of ESLint flat-config builders.
- All `@cybozu/eslint-config/presets/*` entry points now resolve to JSON files. Consumers should reference them via the `extends` field in their own `.oxlintrc.json`.
- Peer dependency changed from `eslint` to `oxlint` (>=1.0.0). `oxlint-tsgolint` is an optional peer for type-aware TypeScript rules (TypeScript 7+ required for type-aware mode).
- **Removed presets** (no direct replacement):
  - `*-prettier` variants — formatting is now handled by [Oxfmt](https://oxc.rs/docs/guide/usage/formatter) or your formatter of choice. These presets contained `eslint-config-prettier` / `eslint-plugin-prettier` integration which is incompatible with Oxlint.
  - `css-baseline` — CSS linting is now out of scope; use [stylelint](https://stylelint.io/).
- **Removed exports**: `./globals/kintone` (kintone globals are now embedded directly in `kintone-customize*` presets).
- **Removed npm dependencies**: `eslint`, `@eslint/js`, `@eslint/css`, `@stylistic/eslint-plugin-ts`, `@typescript-eslint/*`, `eslint-config-prettier`, `eslint-plugin-import-x`, `eslint-plugin-jsx-a11y`, `eslint-plugin-n`, `eslint-plugin-prettier`, `eslint-plugin-react`, `eslint-plugin-react-hooks`, `typescript-eslint`, `globals`, `prettier`, `mocha`, `npm-run-all`.
- **Tooling**: tests migrated from Mocha (JavaScript) to **Vitest + TypeScript**. `tsconfig.json` added. `tsx` is used to run scripts.

### Migration guide

See the [README](./README.md#migration-from-v25-eslint--v26-oxlint) for the consumer-side migration steps.

### Verification

Rule coverage was generated by `@oxlint/migrate --type-aware` and is verified by:

- `scripts/rule-inventory.ts` — dumps every preset's enabled rules to `.rule-inventory/inventory.{json,md}`.
- `scripts/diff-against-baseline.ts` — snapshots and regression-checks rule severity between Oxlint upgrades.
- `test/presets.test.ts` — Vitest suite that validates preset JSON structure and severity sanity.

Skipped rule categories (per `@oxlint/migrate`):

- **Nursery** (experimental in Oxlint): `no-undef`, `react/require-render-return`
- **Not Implemented** (planned): `no-implied-eval`, `require-atomic-updates`, `react/no-typos`, `react/no-unstable-nested-components`, `react/prefer-stateless-function`, `react/jsx-no-bind`
- **Unsupported**: stylistic rules superseded by Oxfmt, legacy class-component React rules, and rules superseded by strict mode / TypeScript `noImplicitThis`.
